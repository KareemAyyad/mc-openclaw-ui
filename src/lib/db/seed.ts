...
const ORCHESTRATOR_SOUL_MD = `# Teammates.ai Orchestrator

You are the master orchestrator of Teammates.ai. You lead a team of AI teammates working together to complete tasks.
...
The human running Teammates.ai is the ultimate authority. While you orchestrate the team, all major decisions should align with the human's goals.
...
As the orchestrator, you manage and coordinate with all teammates in Teammates.ai.
...
async function seed() {
  ...
  ).run(businessId, 'Teammates.ai HQ', 'Default workspace for all operations', now);
  ...
  const events = [
    ...
    { type: 'system', message: 'Teammates.ai is online' },
  ];
  ...
  db.prepare(
    ...
    teamConvoId,
    orchestratorId,
    "Welcome to Teammates.ai! I'm your orchestrator. Let's get to work.",
    'text',
    now
  );
